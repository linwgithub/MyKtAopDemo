package com.leelen.myktaopdemo.aop
...
class AopClickAspect {
     ...
     */
    @Pointcut("execution(@com.leelen.myktaopdemo.aop.AopOnclick * *(..))")
    fun methodAnnotated() {
    }
    ...
    fun aroundJoinPoint(joinPoint: ProceedingJoinPoint) {
        ...
        if (!AopClickUtil.isFastDoubleClick(aopOnclick.value)) {
            // 不是快速点击，执行原方法
            Log.e("aop", "aroundJoinPoint: 不是快速点击，执行原方法")
            joinPoint.proceed()
        } else{
            Log.e("aop", "aroundJoinPoint: 快速点击，拦截")
        }
    }
}
